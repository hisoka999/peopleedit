    ...
    : QObject(parent), d(new QFreeDesktopMime::Private)
{
    // Load Xml Freedesktop
        QFile xml(":/base/freedesktopmime.xml");
...
bool QFreeDesktopMime::checkMatch (const QDomElement& matchNode, QFile *file) {
    ...
    if (type == "string") {
        ...
        if (!mask.isEmpty()) parsedMask = parse_string_mask(mask.toLatin1(), parsedValue.length());
    } else if (type.contains("16")) {
        parse_int_value(2, value.toLatin1(), mask.toLatin1(), &parsedValue, &parsedMask, type[0] != 'l');
